# Requirements Archive: v1.1 Operations & Observability
...
- AUDT-04 (webhook deletion) is partially satisfied — WEBHOOK_REGISTERED and WEBHOOK_UPDATED are wired; WEBHOOK_DELETED enum constant exists but is intentionally unwired because there is no delete endpoint in the v8 API contract. When a delete endpoint is added, the enum is already in place and only the trigger point needs wiring.
- DANL-03 daily breakdown does not include per-day segment totals — consistent with the plan specification; DeliveryDailyStat has date/total_sent/delivered/failed only. Product owner should confirm if per-day segment counts are needed.
